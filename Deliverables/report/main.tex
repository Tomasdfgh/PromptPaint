...
\begin{itemize}
  \item \textbf{Model upgrade to SDXL}: replacing the original Stable Diffusion
        ...
        output resolution from 512$\times$512 to 1024$\times$1024 with
...
\begin{figure*}[!t]
  ...
  embeddings are blended on the palette. (b)~Prompt Palette: a draggable
  cursor sets per-prompt weights; cursor position maps to a mixed embedding
  via Fréchet mean interpolation. (c)~Directional Prompts: ``from'' and
...
The denoising process runs for $K$ steps, with earlier steps establishing
global structure and later steps refining detail. Because each step depends
only on the current latent (the Markov property), it is possible to branch
at any step by substituting a saved latent and continuing denoising with a
different prompt. This temporal structure makes the generation process a
natural target for interactive intervention. SDXL~\cite{podell2023sdxl}
extends this to 1024$\times$1024 resolution with a two-stage pipeline and
2048-dimensional text embeddings, providing richer semantic structure for
prompt manipulation.
...
This allows fine-grained attribute control (e.g., matte $\to$ glossy) without
rewriting the base prompt, and multiple directional prompts can be stacked.
...
\begin{figure}[h]
  ...
  \begin{subfigure}{0.75\linewidth}
    ...
    latent---no timing pressure required.}
  \end{subfigure}
  \vspace{4pt}
  \begin{subfigure}{0.75\linewidth}
...
embedding space before the UNet denoiser. The system is containerized with
...
per generation request). At SDXL's 2048-dimensional embedding scale, this
is negligible compared to UNet inference time.
